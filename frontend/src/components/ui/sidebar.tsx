    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            "group/sidebar-wrapper flex min-h-svh w-full has-[[data-slot=sidebar]]:bg-sidebar",
            className,
          )}
          {...props}
        >
      ...
      <div
        ...
        className={cn(
          ...
          variant === "floating" || variant === "inset"
            ...
            : "group-data-[collapsible=icon]:w-[var(--sidebar-width-icon)] border-sidebar-border border-r",
        )}
      >
        <div
          data-slot="sidebar-inside"
          className={cn(
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "[[data-side=sidebar]_[data-side=sidebar]_&]:cursor-pointer",
        "[[data-side=sidebar]_[data-side=sidebar]_&]:hover:after:bg-sidebar-primary/20",
        "[[data-side=sidebar]_[data-side=sidebar][data-collapsible=offcanvas]_&]:hover:after:bg-sidebar-border",
        "[[data-side=sidebar]_[data-side=sidebar][data-state=collapsed]_&]:cursor-e-resize",
        "[[data-side=sidebar]_[data-side=sidebar][data-state=expanded]_&]:cursor-w-resize",
        className,
      )}
      {...props}
    />
  ...
  return (
    <main
      ...
      className={cn(
        ...
        "peer-data-[variant=inset]:min-h-[calc(100svh-theme(spacing.4))] md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2",
        className,
      )}
      {...props}
    />
  ...
  "peer/menu-button ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden transition-[width,height,padding] focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:font-medium group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 cursor-pointer",
  {
    variants: {
      variant: {
        default: "",
        outline:
          "bg-background shadow-[0_0_0_1px_hsl(var(--sidebar-border))] hover:bg-background hover:shadow-[0_0_0_1px_hsl(var(--sidebar-primary))]",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs",
...
function SidebarMenuButton({
  ...
  if (!isActive && state === "collapsed" && !isMobile) {
    return (
      <Tooltip>
  ...
  return (
    ...
    >
      ...
      <Skeleton
        ...
        style={{ "--skeleton-width": "80px" } as React.CSSProperties}
      />
    </div>
  );
}
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "border-sidebar-border mx-3.5 flex min-w-0 flex-col gap-1 border-l px-2.5",
        "group-data-[collapsible=icon]:hidden",
        className,
      )}
      {...props}
